...
fn resolve(expr: ast::Expr<'_>, environment: &Environment) -> Option<Symbol> {
    match expr {
        ...
        ast::Expr::FuncCall(call) => {
            ...
            (field.field().as_str() == "with"
                && !target.library.rules(&target.path).is_empty())
            .then_some(target)
        }
        _ => None,
    }
...
fn scan(node: &SyntaxNode, depth: usize, count: &mut usize, mutations: &mut HashSet<String>) -> bool {
    *count += 1;
    if depth > 128 || *count > 100_000 {
        return false;
    }
    if let Some(binary) = node.cast::<ast::Binary>() {
        if matches!(binary.op(), ast::BinOp::Assign | ast::BinOp::AddAssign | ast::BinOp::SubAssign | ast::BinOp::MulAssign | ast::BinOp::DivAssign) {
    ...
    node.children().all(|child| scan(child, depth + 1, count, mutations))
}

impl Discovery<'_> {
    fn warn(&mut self, node: &SyntaxNode, message: impl std::fmt::Display) {
        let at = self.source.range(node.span()).map(|span| line(self.source, &span)).unwrap_or(1);
    ...
    fn scope(&mut self, node: &SyntaxNode, inherited: &Environment, top: bool) {
        ...
        let duplicates = counts.into_iter().filter_map(|(name, count)| (count > 1).then_some(name)).collect::<Vec<_>>();
        if !duplicates.is_empty() {
            self.warn(node, format!("Duplicate bindings are read-only: {}", duplicates.join(", ")));
    ...
    fn import(&self, import: ast::ModuleImport<'_>, environment: &mut Environment, blocked: &HashSet<String>) {
    ...
    fn bind(&self, binding: ast::LetBinding<'_>, environment: &mut Environment, blocked: &HashSet<String>) {
        ...
        if let (ast::LetBindingKind::Normal(ast::Pattern::Normal(ast::Expr::Ident(name))), Some(value)) = (binding.kind(), value) {
    ...
    fn binding(&mut self, binding: ast::LetBinding<'_>, environment: &Environment, blocked: &HashSet<String>, top: bool) {
        let Some(init) = binding.init() else { return; };
        if let ast::Expr::FuncCall(call) = init {
            ...
            if claimed {
                ...
                let result = (|| {
                    ...
                    ensure!(symbol.is_some_and(|symbol| symbol.declaration()), "Studio declaration import is unknown, shadowed or rebound");
                    let ast::LetBindingKind::Normal(ast::Pattern::Normal(ast::Expr::Ident(name))) = binding.kind() else {
                        anyhow::bail!("Studio declarations require one binding name");
                    };
                    ensure!(!blocked.contains(name.as_str()), "Duplicate or assigned parameter name is read-only");
    ...
    fn walk(&mut self, node: &SyntaxNode, environment: &Environment) {
        match node.kind() {
            K::Raw | K::Closure | K::ForLoop | K::WhileLoop | K::Conditional | K::Contextual | K::ShowRule | K::SetRule => return,
            K::Markup | K::Code => {
        ...
        if let Some(call) = node.cast::<ast::FuncCall>() {
            let (symbol, preset) = match call.callee() {
                ast::Expr::FieldAccess(field) if field.field().as_str() == "with" => (resolve(field.target(), environment), true),
                callee => (resolve(callee, environment), false),
            };
            if let Some(symbol) = symbol {
                if symbol.declaration() {
                    self.declarations = true;
                    self.warn(node, "studio.param must directly initialize a top-level binding");
                        ...
                        Ok((_, named)) => {
                            for rule in symbol.library.rules(&symbol.path) {
                                if let Some(expr) = named.get(rule.argument) {
                                    match inferred(self.source, call, &symbol, rule, *expr, preset) {
                                        Ok(parameters) => self.inferred.extend(parameters),
                                        Err(error) => self.warn(node, format!("{}.{} is read-only: {error}", symbol.path, rule.argument)),
                                    }
                                }
                            }
                        }
...
fn call_arguments(call: ast::FuncCall<'_>) -> Result<Arguments<'_>> {
    ...
    for item in call.args().items() {
        match item {
            ...
            ast::Arg::Named(arg) => {
                ensure!(named.insert(arg.name().as_str().to_string(), arg.expr()).is_none(), "Duplicate arguments are read-only");
            }
            ast::Arg::Spread(_) => anyhow::bail!("Spread arguments are read-only; effective overrides are not inferred"),
        }
    }
    Ok((positional, named))
}
...
fn explicit(source: &Source, name: &str, call: ast::FuncCall<'_>) -> Result<Parameter> {
    ...
    ensure!(positional.len() == 1, "studio.param requires exactly one literal value");
    ensure!(named.keys().all(|name| ["label", "min", "max", "step", "kind"].contains(&name.as_str())), "Unsupported studio.param option");
    ...
    ensure!(!parameter.label.is_empty() && parameter.label.chars().count() <= 120, "Parameter label must contain 1–120 characters");
    if let Some(kind) = named.get("kind") {
        let ast::Expr::Str(kind) = kind else { anyhow::bail!("Parameter kind must be a string literal"); };
        ...
        ensure!(kind.get().as_str() == expected, "Declared kind does not match the literal kind");
    }
    let bound = |name: &str| -> Result<Option<f64>> {
        named.get(name).map(|expr| {
            let bound = literal(source, *expr)?;
            ensure!(bound.kind == parameter.kind && bound.unit == parameter.unit, "{name} must use the same unit as the value");
            bound.value.as_f64().context("Bounds require numeric values")
        }).transpose()
    };
    ...
    if let Some(step) = parameter.step { ensure!(step > 0.0, "step must be positive"); }
    if let Some(value) = parameter.value.as_f64() {
        ensure!(parameter.min.is_none_or(|min| value >= min), "Value is below min");
        ensure!(parameter.max.is_none_or(|max| value <= max), "Value is above max");
    }
    Ok(parameter)
}

fn array(expr: ast::Expr<'_>, count: usize) -> Result<Vec<ast::Expr<'_>>> {
    let ast::Expr::Array(array) = expr else { anyhow::bail!("Expected a literal tuple"); };
    let items = array.items().map(|item| match item {
...
fn inferred(source: &Source, call: ast::FuncCall<'_>, symbol: &Symbol, rule: &Rule, expr: ast::Expr<'_>, preset: bool) -> Result<Vec<Parameter>> {
    let leaves = match rule.shape {
        Shape::Scalar => vec![(String::new(), expr)],
        Shape::Pair | Shape::Triple => array(expr, if matches!(rule.shape, Shape::Pair) { 2 } else { 3 })?.into_iter().enumerate().map(|(i, expr)| (format!("[{i}]"), expr)).collect(),
        Shape::TwoTriples => {
            let mut leaves = vec![];
    ...
    let at = source.range(call.span()).context("Call has no source range")?.start;
    leaves.into_iter().map(|(index, expr)| {
        let mut parameter = literal(source, expr)?;
        ensure!(rule.scalar.accepts(&parameter), "Literal does not use the reviewed argument type/units");
        let value = parameter.value.as_f64().context("Expected numeric display property")?;
        ensure!(value >= rule.min, "Original value is outside the supported display range");
        let argument = format!("{}{index}", rule.argument);
        parameter.id = format!("library:{at}:{}:{argument}", symbol.path);
        parameter.label = format!("{} · {argument} · line {}{}", symbol.path, parameter.line, if preset { " (preset)" } else { "" });
