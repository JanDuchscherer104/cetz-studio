...
fn compiler(root: PathBuf) -> Compiler {
    Compiler {
        executable: std::env::var_os("TYPST").map(PathBuf::from).unwrap_or_else(|| "typst".into()),
        root,
...
fn native_library_controls_round_trip() {
    let fixtures = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/library-controls");
    ...
    for (name, argument, value, old, replacement) in [
        ...
        ("scenery", "azimuth", 50.0, "azimuth: 30deg", "azimuth: 50deg"),
        ...
        fs::copy(fixtures.join("tetra.obj"), temporary.path().join("tetra.obj")).unwrap();
        let compiler = compiler(temporary.path().to_path_buf());
        let mut session = Session::open(file.clone(), compiler.clone(), None).unwrap();
        session.render().unwrap_or_else(|error| panic!("{name}: {error:#}"));
        let before = session.snapshot();
        assert!(before.preview_current, "{name}");
        assert_eq!(before.pages.len(), 1, "{name}");
        let parameter = before.parameters.iter().find(|p| p.origin.as_ref().is_some_and(|origin| origin.argument == argument)).unwrap_or_else(|| panic!("{name}: {:?}", before.warnings));
        let id = parameter.id.clone();
        session.edit(0, Command::SetParameter { id: id.clone(), value: parameter.value.clone() }).unwrap();
        assert_eq!(session.revision, 0, "{name}: no-op changed revision");
        assert!(session.save(0).unwrap().is_none(), "{name}: no-op created backup");
        session.edit(0, Command::SetParameter { id: id.clone(), value: Value::from(value) }).unwrap();
        let after = session.snapshot();
        let expected = original.replacen(old, replacement, 1);
        assert_eq!(after.source, expected, "{name}: nonlocal source change");
        assert_ne!(after.pages, before.pages, "{name}: edit had no rendered effect");
        assert_eq!(fs::read_to_string(&file).unwrap(), original, "{name}: draft touched disk");
        assert!(session.edit(0, Command::SetParameter { id, value: Value::from(value) }).is_err());
        ...
        assert_eq!(fs::read_to_string(backup).unwrap(), original, "{name}: backup");
        ...
        assert!(reopened.snapshot().parameters.iter().any(|p| p.origin.as_ref().is_some_and(|origin| origin.argument == argument) && p.value == Value::from(value)));
        assert_eq!(fs::read(temporary.path().join("tetra.obj")).unwrap(), fs::read(fixtures.join("tetra.obj")).unwrap());
        ...
        fs::write(output.join(format!("{name}-controls.json")), serde_json::to_vec_pretty(&before.parameters).unwrap()).unwrap();
    }
}
...
fn invalid_library_edit_preserves_accepted_source_and_render() {
    ...
    let mut session = Session::open(file.clone(), compiler(temporary.path().to_path_buf()), None).unwrap();
    session.render().unwrap();
    let before = session.snapshot();
    let parameter = before.parameters.iter().find(|p| p.origin.as_ref().is_some_and(|origin| origin.argument == "length")).unwrap();
    assert!(session.edit(0, Command::SetParameter { id: parameter.id.clone(), value: Value::from(-1) }).is_err());
    ...
    let before = session.snapshot();
    let parameter = before.parameters.iter().find(|p| p.origin.as_ref().is_some_and(|origin| origin.argument == "length")).unwrap();
    fs::write(temporary.path().join("dependency.typ"), "#panic(\"failed dependency\")").unwrap();
    assert!(session.edit(0, Command::SetParameter { id: parameter.id.clone(), value: Value::from(10) }).is_err());
    assert_eq!(session.snapshot().source, before.source);
    assert_eq!(session.snapshot().pages, before.pages);
