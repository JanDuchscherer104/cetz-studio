...
fn renamed_module_and_named_parameter_imports_work() {
    for source in [
        ...
        assert_eq!(
            apply(source, &parsed.parameters, "x", &Value::from(3)).unwrap(),
            source.replacen("(2)", "(3)", 1)
        );
    }
}
...
fn angles_ratios_and_relative_lengths_preserve_authored_units() {
    for (value, next, expected) in [
        ...
        assert_eq!(
            apply(&source, &parsed.parameters, "x", &Value::from(next)).unwrap(),
            source.replace(value, expected)
        );
    }
}
...
fn parenthesized_literals_and_unicode_comments_survive() {
    let source =
        format!("{PREFIX}// αβ 日本語\n#let x = studio.param(( /* keep */ -2.5 /* too */ ))\n[µ]");
    let parsed = parse(&source);
    assert_eq!(parsed.parameters.len(), 1, "{:?}", parsed.warnings);
    assert_eq!(
        apply(&source, &parsed.parameters, "x", &Value::from(-3.0)).unwrap(),
        source.replace("-2.5", "-3")
    );
}
...
fn stale_discovery_is_rejected_even_for_a_no_op() {
    ...
    .is_err());
    assert!(apply(
        &source.replace("param(2)", "param(3)"),
        &parsed.parameters,
        "x",
        &Value::from(4)
    )
...
fn automatic_camera_controls_need_no_studio_import() {
    ...
    let camera = parsed
        .parameters
        .iter()
        .find(|p| p.origin.as_ref().unwrap().argument == "azimuth")
        .unwrap();
    assert_eq!(camera.unit.as_deref(), Some("deg"));
    assert_eq!(
        apply(&source, &parsed.parameters, &camera.id, &Value::from(-35)).unwrap(),
        source.replace("-38deg", "-35deg")
    );
}
...
fn aliases_named_imports_and_with_presets_keep_provenance() {
    ...
    assert_eq!(
        parsed.parameters[0].origin.as_ref().unwrap().function,
        "camera"
    );
...
fn malformed_declarations_do_not_fall_back_to_implicit_controls() {
    let source = format!(
        "{PREFIX}{SCENERY}#let az = studio.param(10deg + 20deg)\n#scene.camera(azimuth: 50deg)"
    );
    assert!(parse(&source).parameters.is_empty());
}
...
fn plotsy_tuples_are_leaf_edits_not_reprinted_expressions() {
    ...
    let parameter = parsed
        .parameters
        .iter()
        .find(|p| p.origin.as_ref().unwrap().argument == "scale-dim[1]")
        .unwrap();
    ...
    assert!(parsed
        .parameters
        .iter()
        .all(|p| !p.label.contains("xdomain")));
}
...
fn malformed_or_computed_tuples_are_wholly_read_only() {
    for tuple in ["(1, 2)", "(1, value, 3)", "(1, ..rest)"] {
        let source = format!(
            "#import \"@preview/plotsy-3d:0.2.1\" as p\n#p.plot-3d-surface(f, scale-dim: {tuple})"
        );
        assert!(parse(&source).parameters.is_empty(), "{source}");
    }
}
...
fn maquette_numeric_angles_are_not_conflated_with_scenery_angles() {
    ...
    assert!(parse(&format!("{SCENERY}#scene.camera(azimuth: 30)"))
        .parameters
        .is_empty());
}
...
fn fletcher_layout_arguments_preserve_rich_node_content() {
    ...
    let parameter = parsed
        .parameters
        .iter()
        .find(|p| p.origin.as_ref().unwrap().argument == "width")
        .unwrap();
    assert_eq!(
        apply(source, &parsed.parameters, &parameter.id, &Value::from(27)).unwrap(),
        source.replace("25mm", "27mm")
    );
}
...
fn parameter_and_traversal_budgets_fail_closed() {
    let source = format!(
        "{PREFIX}{}",
        (0..257)
            .map(|i| format!("#let p{i} = studio.param(1)\n"))
            .collect::<String>()
    );
    assert!(parse(&source).parameters.is_empty());
    let source = format!(
        "{SCENERY}#{}scene.camera(azimuth: 30deg){}",
        "[".repeat(150),
        "]".repeat(150)
    );
    assert!(parse(&source).parameters.is_empty());
...
fn syntax_errors_disable_all_controls() {
    assert!(parse(&format!("{SCENERY}#scene.camera(azimuth: 30deg"))
        .parameters
        .is_empty());
}

#[test]
fn later_rebinding_of_a_renamed_declaration_does_not_widen_controls() {
    let source = "#import \"@local/cetz-studio:0.1.0\": param as control\n#import \"@preview/scenery:0.1.0\" as scene\n#let x = control(2)\n#let control = other\n#scene.camera(azimuth: 30deg)";
    assert!(parse(source).parameters.is_empty());
}

#[test]
fn nonfinite_and_inexact_integer_controls_are_read_only() {
    for value in ["9007199254740993", "99999999999999999999999999", "1e999"] {
        let source = format!("{PREFIX}#let x = studio.param({value})");
        assert!(parse(&source).parameters.is_empty(), "{value}");
    }
}
